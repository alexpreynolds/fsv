      ...
      case RETURN_KEY: {
        ...
        setTimeout(() => {
          ...
          if (indexDHSIdentifierTest) {
            ...
            axios.get(mapUrl)
              .then((res) => {
                if (res.data.data) {
                  ...
                  const indexDHSLocation = `${hit.seqname}:${hit.start - Constants.appDefaultHgViewIndexDHSPadding}-${hit.end + Constants.appDefaultHgViewIndexDHSPadding}`;
            ...
            if (chromosomeRange) {
              // console.log(`chromosomeRange ${chromosomeRange}`);
              const chromosomeRangeLocation = `${chromosomeRange[0]}:${chromosomeRange[1]}-${chromosomeRange[2]}`;
              this.setState({
                activeSuggestion: -1,
                filteredSuggestions: [],
                showSuggestions: false,
            ...
            if (interval) {
              ...
              const intervalRangeLocation = `${interval[0]}:${interval[1]}-${interval[2]}`;
              this.setState({
                activeSuggestion: -1,
                filteredSuggestions: [],
                showSuggestions: false,
